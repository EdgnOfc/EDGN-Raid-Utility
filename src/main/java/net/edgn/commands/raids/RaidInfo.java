...
public class RaidInfo extends CommandBase {
	@Override
	...
	final Thread Threaded = new Thread() {
		public void run() {
				...
				ChatUtils.message(player, tag + " " + ChatFormatting.GOLD + " " + "Raid completions of : " + " " + ChatFormatting.RED + ChatFormatting.BOLD + pseudo);
				ChatUtils.message(player, " ");
				String result = WynnApiUtils.getStringFromURL("https://api.wynncraft.com/v2/player/" + pseudo + "/stats");
				int completion = 0;
				for (int i = 0; i < raid_name.size(); i++) {
					...
					ChatUtils.message(player,
							tag + " " +  ChatFormatting.AQUA + ChatFormatting.BOLD +  raid_libelle.get(i) + " : " + ChatFormatting.GOLD + FinalCompletion.get(i) + " " +  ChatFormatting.YELLOW +  " times.");
				}
				int sum = FinalCompletion.stream().mapToInt(i -> i).sum();
				ChatUtils.message(player, tag +  " " +  ChatFormatting.AQUA + ChatFormatting.BOLD + "Total : " + ChatFormatting.GOLD + sum + " " +  ChatFormatting.YELLOW +  " Times.");
				ChatUtils.message(player, StringUtils.repeat("§m ", 60));
			}
		};
		Threaded.start();
